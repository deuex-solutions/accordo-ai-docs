...
A total of **21 orphan link flags** were raised by the crawl tool (with several duplicates — 14 unique routes after deduplication). After manual investigation of the codebase, **none of them are genuinely orphan routes**. They fall into three categories:
...
The following routes were flagged multiple times during the crawl. They are only analyzed once in this report:
...
## JS Errors Detected During Crawl

The crawl tool also flagged JavaScript errors encountered on specific pages. These are analyzed below.

---

### JS Error 1: `Error fetching company data: AxiosError`

**Page:** `http://localhost:5001/vendor-management/edit-vendor/17?step=2`

**Error Type:** API request failure during component data fetch

**Error Location:** `src/components/vendorForm/VendorFormContainer.tsx`, line 174
...
useEffect(() => {
  const fetchCompanyData = async () => {
    if (companyId && !propCompany) {
      try {
        const response = await authApi.get(`/company/${companyId}`); // GET /api/company/17
        setCompany(response.data.data);
      } catch (error) {
        console.error("Error fetching company data:", error); // <-- Error logged here
...
**What happens:** When editing vendor ID 17, the component mounts and fetches company data from `GET /api/company/17`. The `?step=2` query param only controls which form step is displayed (Step 2 = Location Details) — it does not affect the API call.

**Possible Causes (in order of probability):**

| Cause                                        | HTTP Status   | How to Verify                         |
| -------------------------------------------- | ------------- | ------------------------------------- |
| Company ID 17 doesn't exist in the database  | 404           | Check database for company with ID 17 |
| Authentication token expired/missing         | 401           | Check Network tab for response status |
| Backend not running on port 5002             | Network Error | Verify backend is running             |
| Database association error (Sequelize joins) | 500           | Check backend logs for SQL errors     |

**Backend Flow:**

1. `GET /api/company/17` → `company.controller.ts` → `getCompanyService(17)` → `companyRepo.getCompany(17)`
2. The repo uses Sequelize `findByPk(17)` with `include` joins across `Company`, `User`, `VendorCompany`, and `Address` tables
3. If any association is misconfigured or the company doesn't exist, the request fails

**How to Debug:**

- Open browser **Network tab** → look for `GET /api/company/17` → check the HTTP status code
- If 404: seed the database with company ID 17 or use a valid vendor ID
- If 401: re-authenticate (token expired)
- If 500: check backend console logs for the full error stack trace

**Verdict:** This is likely a **data issue** (company 17 doesn't exist in the test environment) or an **authentication issue** (crawl tool isn't authenticated). Not a code bug.

---

### JS Error 2: `Request failed with status code 401`

**Page:** `http://localhost:5001/setting`

**Error Type:** Authentication failure on protected API endpoint

**What happens:** The Settings page (`UserInfo.tsx`) makes authenticated API calls using the `authApi` instance to fetch user profile and company data. The backend rejects the request with HTTP 401 because no valid authentication token is present.

**Root Cause:** The crawl tool is accessing a protected page without being authenticated. The expected flow is:

1. Page component mounts → attempts API calls with `authApi`
2. No valid token in localStorage (`%accessToken%`) → backend returns 401
3. `authApi` interceptor attempts token refresh using `%refreshToken%`
4. Refresh also fails (no valid refresh token) → error surfaces as JS error
5. User would normally be redirected to `/sign-in`

**This is expected behavior, not a bug.** The app is correctly rejecting unauthenticated access. A real user would either:

- Already be authenticated (token in localStorage), or
- Be redirected to the login page by the `ProtectedRoute` guard

**Verdict:** Not a code issue. This error will appear on **every protected route** the crawl tool visits without authentication. The fix is to configure the crawl tool to authenticate before scanning.

**Affected Routes (all will produce 401 if unauthenticated):**
...
**No code changes are required.** Both errors are caused by the crawl tool accessing protected pages without authentication:

- **Error 1** (`/vendor-management/edit-vendor/17?step=2`): Either missing auth token or non-existent test data (company ID 17)
- **Error 2** (`/setting`): Missing auth token on a protected route — expected behavior

### Recommendations for the Crawl Tool

The crawl tool would benefit from:

1. **Authenticated scanning** to access protected routes and avoid 401 errors
...
4. **Test data seeding** — ensure the database contains valid records for any routes with dynamic IDs being tested

These improvements would eliminate all 21 orphan false positives and both JS errors identified in this report.
